// Const docs
const numPad = document.querySelectorAll('.num-1, .num-2, .num-3, .num-4, .num-5, .num-6, .num-7, .num-8, .num-9, .num-0');
let visibility = false;

const pi = "";
let addnum = "";
let index = 0;

// visibility animation
document.querySelector('.material-symbols-outlined').addEventListener('click', () => {
    document.querySelector('.material-symbols-outlined').classList.toggle('clicked');

    if (document.querySelector('.material-symbols-outlined').classList.contains('clicked')) {
        console.log('TRU TRU');
        visibility = true;
}    else visibility = false;
});

// Reset
document.querySelector('.cross').addEventListener('click', () => {
    document.querySelector('.cross').classList.toggle('clicked');
    showVisibility(index);
    restart();
});

document.querySelector('.cross').addEventListener('animationend', () => {
    document.querySelector('.cross').classList.toggle('clicked');
});

// Sequencing
window.addEventListener('keydown', 
    function handleNumAccept(e) {
        if (e.key === pi[index]) {
            addnum += e.key;
            index++;
        }

        showVisibility(index);

        document.querySelector('.sequence-after').textContent = addnum;
    }
);

function showVisibility(indx) {
    if (visibility)
    document.querySelector('.clue-text').textContent = pi[index];
else document.querySelector('.clue-text').textContent = "";
}

function restart() {
    index = 0;
    addnum = " ";
    document.querySelector('.sequence-after').textContent = "";
}

showVisibility(index);